...
class Jpy : public Currency{
    public:
      Jpy(const std::string& abbreviation, double amount) : abbreviation_(abbreviation), amount_(amount) {}
...
int main() {
    FakeUsdCantor usd;
    Jpy first("JPY", 1000.0);
    std::cout<<"to usd: "<<first.ConvertedToDollars(usd)<<std::endl;
    std::cout<<"initial amount: "<<first.Amount()<<std::endl;
    std::cout<<"abb: "<<first.Abbreviation()<<std::endl;
     return 0;
}
